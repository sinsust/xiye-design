...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const items: { id?: string; action?: string; overrides?: InboxOverrides }[] = Array.isArray(body?.items)
      ? body.items
      : [];
    const results = [];
    let processed = 0;
    ...
    return NextResponse.json({ error: "batch_failed", detail: String(err) }, { status: 500 });
  }
}