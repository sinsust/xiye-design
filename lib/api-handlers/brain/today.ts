...
// 对应会话决策：两套复习（SM-2 间隔复习 + 学习复习）收敛为今日卡片，≤5 条；
// 数据库无新增表，仅复用既有查询。复习功能于 10a8b09 移除后按 PRD §7.4 恢复。
...
export async function GET() {
  ...
  try {
    ...
    const todayReviews = cards.slice(0, REVIEW_CARD_LIMIT);
