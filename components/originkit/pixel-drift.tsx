...
// Pixel Drift — 粒子漂移字
// 文字被渲染成密集的彩色粒子场，光标像黑洞在星场上挖出空洞：落入光标半径内的
// 粒子被推挤到半径边缘的圆环上，圆环之外则停留在原位。光标静止 = 空洞静止。
// 组件自持 rAF 帧循环，纯 Canvas 2D，零依赖；重建自 Originkit ParticleText。
// 触发模式 onHover（悬停成型/移出消散）或 onEnter（滚入视口成型）。

import * as React from "react"
import { useEffect, useRef } from "react"

// ── Transition helpers ───────────────────────────────────────────────────────
// 用 Framer Transition 值驱动成型动画：duration 决定粒子聚集成文字的时间，
// ease 决定聚入时的曲线。
...
// 求值 CSS cubic-bezier [x1,y1,x2,y2] 为 easing 函数。
...
export default function ParticleText(props: Partial<Props>) {
    ...
    return <__OriginkitBase_ParticleText {...merged} />
}
...
function __OriginkitBase_ParticleText(props: Props) {
    ...
    // Animated formation value 0→1 (0 = 出生位 / 不可见，1 = 完全成型)。
    // 基于速率而非时间轴，使出现与消散可被打断并从当前部分状态继续。
    ...
    const mcRadius = typeof mouseRadius === "number" ? mouseRadius : 50
    const mcForce = typeof mouseForce === "number" ? mouseForce : 30
    ...
    useEffect(() => {
        ...
        const sampleText = () => {
            ...
            // Particle Count（1–50）：更大越密。采样间隔 gap = 150 / count。
        ...
        const drawFrame = () => {
            ...
            // 绘制方块尺寸（~size/4），使 1–100 范围保持合理。
            const drawSize = Math.max(1, particleSize / 4)
            const half = drawSize / 2

            // 依据 Transition duration 以速率推进成型值 0→1。翻转方向（悬停离开、
            // replay 退场）时从当前位置继续，而非瞬跳到完全成型。
            const now =
