'use client'

// next/image stripped
import styles from './services-comparison.module.css';

export default function Comparison() {
  return (
      <section className={styles.comparison}>
        <div className={styles.r1}>
          <div className={styles.r2}>
            <div className={styles.dektop}>
              <div className={styles.tag} data-border="true">
                <div className={styles.icon}>
                  <div className={styles.r6}>
                    <img className={styles.r7} src="/originkit/outstand/services-comparison/2389ppijLGOSFNlbVEzTnaSo.svg" alt="Icon" width={21} height={20} />
                  </div>
                </div>
                <div className={styles.text}>
                  <p className={styles.r9}>
                    Comparison
                  </p>
                </div>
              </div>
              <div className={styles.textContainer}>
                <div className={styles.heading}>
                  <h2 className={styles.r12}>
                    Service Comparison Overview
                  </h2>
                </div>
                <div className={styles.paragraph}>
                  <p className={styles.r14}>
                    Easily compare our services side-by-side to find the perfect fit for your needs. Whether you&apos;re looking for customization options, scalability, or security features, etc.
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className={styles.strokeContainer} data-border="true">
          <div className={styles.scrollContainer}>
            <div className={styles.table} data-border="true">
              <div className={styles.row}>
                <div className={styles.column1} data-border="true">
                  <div className={styles.heading2}>
                    <h3 className={styles.r21}>
                      Features
                    </h3>
                  </div>
                </div>
                <div className={styles.column1} data-border="true">
                  <div className={styles.heading3}>
                    <h3 className={styles.r23}>
                      Basic Package
                    </h3>
                  </div>
                </div>
                <div className={styles.column3} data-border="true">
                  <div className={styles.heading4}>
                    <h3 className={styles.r26}>
                      Standard Package
                    </h3>
                  </div>
                </div>
                <div className={styles.column3} data-border="true">
                  <div className={styles.heading4}>
                    <h3 className={styles.r26}>
                      Premium package
                    </h3>
                  </div>
                </div>
                <div className={styles.column3} data-border="true">
                  <div className={styles.heading4}>
                    <h3 className={styles.r26}>
                      Custom Package
                    </h3>
                  </div>
                </div>
              </div>
              <div className={styles.row2}>
                <div className={styles.column12} data-border="true">
                  <div className={styles.heading5}>
                    <p className={styles.r30}>
                      Web Design
                    </p>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
              </div>
              <div className={styles.row3}>
                <div className={styles.column13} data-border="true">
                  <div className={styles.heading6}>
                    <p className={styles.r38}>
                      Digital Marketing
                    </p>
                  </div>
                </div>
                <div className={styles.column22} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column22} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column22} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column22} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
              </div>
              <div className={styles.row2}>
                <div className={styles.column12} data-border="true">
                  <div className={styles.heading5}>
                    <p className={styles.r30}>
                      Branding
                    </p>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/SLuIZXA4KbIw2wd9hyqsEtWGDE.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/SLuIZXA4KbIw2wd9hyqsEtWGDE.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
              </div>
              <div className={styles.row2}>
                <div className={styles.column12} data-border="true">
                  <div className={styles.heading5}>
                    <p className={styles.r30}>
                      SEO
                    </p>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/SLuIZXA4KbIw2wd9hyqsEtWGDE.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
              </div>
              <div className={styles.row3}>
                <div className={styles.column13} data-border="true">
                  <div className={styles.heading6}>
                    <p className={styles.r38}>
                      Content Strategy
                    </p>
                  </div>
                </div>
                <div className={styles.column22} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/SLuIZXA4KbIw2wd9hyqsEtWGDE.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column22} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/SLuIZXA4KbIw2wd9hyqsEtWGDE.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column22} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column22} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
              </div>
              <div className={styles.row2}>
                <div className={styles.column12} data-border="true">
                  <div className={styles.heading5}>
                    <p className={styles.r30}>
                      UI Design
                    </p>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/SLuIZXA4KbIw2wd9hyqsEtWGDE.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
              </div>
              <div className={styles.row2}>
                <div className={styles.column12} data-border="true">
                  <div className={styles.heading5}>
                    <p className={styles.r30}>
                      UX Research
                    </p>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/SLuIZXA4KbIw2wd9hyqsEtWGDE.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/SLuIZXA4KbIw2wd9hyqsEtWGDE.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/SLuIZXA4KbIw2wd9hyqsEtWGDE.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column2} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
              </div>
              <div className={styles.row3}>
                <div className={styles.column14} data-border="true">
                  <div className={styles.heading6}>
                    <p className={styles.r38}>
                      Security Features
                    </p>
                  </div>
                </div>
                <div className={styles.column23} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/SLuIZXA4KbIw2wd9hyqsEtWGDE.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column23} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/SLuIZXA4KbIw2wd9hyqsEtWGDE.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column23} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/SLuIZXA4KbIw2wd9hyqsEtWGDE.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
                <div className={styles.column23} data-border="true">
                  <div className={styles.tickIcon}>
                    <div className={styles.r33}>
                      <img className={styles.r34} src="/originkit/outstand/services-comparison/oUftBL1miexZcpuwoI9ULejss.svg" alt="Icon" width={100} height={100} />
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>
  );
}

