  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5 shadow-sm">
      <h2 className="flex items-center gap-2 text-sm font-semibold text-foreground">
        ...
        今天到期 / 已逾期
        <span className="rounded-full bg-warning/10 px-1.5 py-0.5 text-[11px] font-semibold text-warning">
          {items.length}
        </span>
      </h2>
      <ul className="mt-3 space-y-1.5">
        {items.map((t) => {
          ...
          return (
            ...
            >
              <span className={"size-1.5 shrink-0 rounded-full " + (overdue ? "bg-danger" : "bg-warning")} />
